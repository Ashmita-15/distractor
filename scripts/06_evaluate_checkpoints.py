...
def main() -> None:
    ...
    parser.add_argument("--run-tag", type=str, default="",
                        help="Prefix for output filenames and the test-evaluation "
                             "artifact tag. Required when several runs share an "
                             "outputs/ directory (e.g. multi-seed replication: "
                             "--run-tag exp3_seed7_) so results do not overwrite "
                             "each other. Default '' reproduces Experiments 2-3.")
    ...
    csv_path = RESULTS_DIR / f"{args.run_tag}checkpoint_metrics_val.csv"
    ...
    # The tag carries run_tag so per-query retrieved indices are preserved per
    # run ({tag}_indices.npy) — required for per-query significance testing.
    sel_test = run_retrieval_evaluation(
        model_name_or_path=str(best_path), tag=f"{args.run_tag}selected",
    ...
    final_csv = RESULTS_DIR / f"{args.run_tag}final_test_results.csv"
...
def write_report(df, identity, best_step, base_test, sel_test, final_df,
                 experiment_name: str = "Experiment 2", compare_df=None,
                 run_tag: str = "") -> None:
    ...
    report_path = RESULTS_DIR / f"{run_tag}checkpoint_trajectory_report.md"
